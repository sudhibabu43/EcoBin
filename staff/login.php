<?php 
 ...
 session_start();
  ...
  if(isset($_REQUEST['submit'])){
   $aEmail = mysqli_real_escape_string($conn, trim($_REQUEST['email']));
   $aPassword = mysqli_real_escape_string($conn, trim($_REQUEST['password']));
   $sql = "SELECT email,Password FROM staff_login WHERE email = '".$aEmail."' AND Password = '".$aPassword."'";
   $result = $conn->query($sql);
   if($result->num_rows == 1){
    $_SESSION['is_stafflogin'] = true;
    $_SESSION['semail'] = $aEmail;
  ...
  <div class="row justify-content-center custom-margin">
   <div class="col-sm-6 col-md-4">
    <form action="" class="shadow-lg box" method="POST">
        <p class="text-center box-text" style="font-size:30px;"><i class="fas fa-user-secret text-success"></i>Staff Login</p>    
